...
function tratar_anexo($anexo) {
    ...
    $dirUploads = "anexos"; //nome da pasta

    if (!is_dir($dirUploads)) { //se não existe então cria ou cancela
        mkdir($dirUploads) or die ("Não foi possível criar a pasta " . $dirUploads);
    }
    if (move_uploaded_file($anexo['tmp_name'], $dirUploads . DIRECTORY_SEPARATOR . $anexo['name'])) {//move arquivo para esse diretorio
        return true;
    } else {
        return false;
    }
